...
export const AddPasswordScreen: React.FC = () => {
  ...
  const save = async () => {
    if (!title.trim() || !username.trim() || !password.trim()) {
      Alert.alert('Missing details', 'Title, username and password are required.');
      return;
    }

    setSaving(true);
    try {
      await vaultFirestoreService.savePassword({
        title,
        username,
        password,
        website,
  ...
  return (
    <GradientPageView scroll>
      <View style={styles.content}>
        <AppHeader title="Add Password" leftIcon={KeyRound} />
        ...
        <Pressable style={styles.generateButton} onPress={() => setPassword(passwordService.generatePassword())}>
